...
export default function NismaraPlusBadge({ className = "w-4 h-4" }: { className?: string }) {
  return (
    <div className="group/nplus relative inline-flex items-center justify-center cursor-help ml-1.5 align-middle">
      {/* Glow effect */}
      <div className="absolute inset-0 bg-yellow-400/50 blur-sm rounded-full scale-150 opacity-0 group-hover/nplus:opacity-100 transition-opacity duration-300" />
      <svg 
        xmlns="http://www.w3.org/2000/svg" 
        viewBox="0 0 512 438" 
        className={`${className} fill-yellow-400 drop-shadow-[0_0_4px_rgba(250,204,21,0.6)] relative z-10 transition-transform duration-300 group-hover/nplus:scale-110`}
      >
      ...
      {/* Tooltip */}
      <div className="absolute top-full left-1/2 -translate-x-1/2 mt-1.5 px-2 py-1 bg-black/95 text-yellow-400 text-[10px] font-bold rounded-md opacity-0 group-hover/nplus:opacity-100 transition-all duration-300 -translate-y-1 group-hover/nplus:translate-y-0 whitespace-nowrap pointer-events-none border border-yellow-400/30 shadow-[0_0_10px_rgba(250,204,21,0.2)] z-[9999]">
        Nismara Plus Member
      </div>
    </div>
  );
