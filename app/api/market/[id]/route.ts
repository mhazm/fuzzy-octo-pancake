...
export async function PUT(
  ...
  try {
    ...
    const isPriceChanged = data.price !== undefined && Number(data.price) !== item.price;
    ...
    if (data.slug && data.slug !== item.slug) {
      ...
      if (existingSlug) {
        return NextResponse.json({ error: "Slug/URI sudah digunakan oleh mod lain" }, { status: 400 });
      }
      item.slug = data.slug;
    }
    ...
    if (isPriceChanged) {
      ...
      try {
        ...
        const safeUsername = session.user.name?.replace(/[^a-zA-Z0-9]/g, "").toLowerCase() || "driver";
          ...
          await fetch(`https://discord.com/api/v10/channels/${channelData.id}/messages`, {
            ...
            body: JSON.stringify({
              ...
              embeds: [{
                ...
                color: 0xf59e0b,
                fields: [
                  { name: "Harga Baru", value: `${item.price} NC`, inline: true },
                ]
              }]
            })
          });
        ...
        console.error("Failed to create discord ticket for re-approval:", discordErr);
      }
    }
    ...
    try {
      const purchases = await MarketPurchase.find({ marketItemId: item._id }).lean();
    ...
    return NextResponse.json(
      { success: true, message: "Mod berhasil diupdate!", data: item },
    );
  } catch (error) {
...
export async function DELETE(
  ...
  try {
    ...
    if (!isOwner && !isManager) {
      return NextResponse.json(
        { error: "Forbidden: Hanya pemilik mod atau Manajemen Nismara yang dapat menghapus" },
        { status: 403 },
      );
    }

    // Ambil data semua pembeli sebelum item dihapus
    const purchases = await MarketPurchase.find({ marketItemId: dbItem._id }).lean();
    ...
    const buyerNotifications = purchases
      ...
      .map((p: any) => ({
        recipient: p.buyerId,
        title: isOwner ? "🗑️ Mod Dihapus Kreator" : "🛡️ Mod Ditarik oleh Manajemen",
        message: isOwner 
    ...
    return NextResponse.json(
      { success: true, message: "Mod berhasil dihapus!" },
    );
  } catch (error) {
