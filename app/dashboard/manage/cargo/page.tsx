...
export default function CargoManager() {
  ...
  const handleEditClick = (cargo: any) => {
    setEditData({ id: cargo.id, name: cargo.name, price: cargo.price_per_km });
    setIsModalOpen(true);
  };
  ...
  const handleSave = async () => {
    ...
    try {
      const res = await fetch(`/api/cargo/${editData.id}`, {
        ...
        body: JSON.stringify({ price_per_km: parseFloat(editData.price) }),
      });
      ...
      setCargoes(
        cargoes.map((c) =>
          c.id === editData.id
            ? { ...c, price_per_km: parseFloat(editData.price) }
            : c,
        ),
      );
          ...
          <div className="flex gap-2">
            ...
            <div className="relative flex items-center">
              ...
              <select
                ...
                className="bg-white/5 border border-border rounded-2xl py-4 pl-10 pr-8 text-foreground focus:outline-none focus:border-accent-lilac appearance-none font-bold text-xs uppercase tracking-widest cursor-pointer"
              >
            ...
            <table className="w-full text-left border-collapse relative">
              <thead>
                <tr className="text-foreground/20 text-[10px] font-black uppercase tracking-widest border-b border-border bg-card/80 sticky top-0 z-10 backdrop-blur-md">
                  ...
                  <th className="px-8 py-5 text-right">Base Price / KM</th>
