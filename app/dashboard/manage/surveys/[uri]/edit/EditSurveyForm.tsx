...
export default function EditSurveyForm({
  ...
  initialData: SurveyFormValues & { imageUrl?: string };
}) {
  ...
  async function onSubmit(data: SurveyFormValues) {
    ...
    try {
      ...
      } else if (!bannerPreview && initialData.imageUrl) {
        finalImageUrl = "";
      }

      const response = await updateSurveyAction({
        ...data,
        imageUrl: finalImageUrl,
      });
      if (response.success) {
        await showAlert("✅ Berhasil! Perubahan survey telah disimpan.");
  ...
  const activeStatus = form.watch("active");
      ...
      <div className="flex items-center justify-between gap-4">
        ...
        <Form {...form}>
          <FormField
            ...
            name="active"
            render={({ field }) => (
              <FormItem className="flex items-center gap-2.5 space-y-0 bg-card border border-border px-4 py-2 rounded-lg shadow-sm shrink-0">
                ...
                <span
                  className={`text-xs font-bold uppercase tracking-wider ${
                    field.value ? "text-accent-sky" : "text-muted-foreground"
                  }`}
                >
                  {field.value ? "Aktif" : "Ditutup"}
                </span>
              </FormItem>
            )}
          />
                  ...
                  <FormItem className="max-w-sm">
                    ...
                    <Select onValueChange={field.onChange} value={field.value}>
                      ...
                      <SelectContent>
                        ...
                        <SelectItem value="nismara_plus">
                          ⭐ Khusus Nismara+ Aktif
                        </SelectItem>
                        <SelectItem value="intern">
                          🔰 Khusus Driver Intern
                        </SelectItem>
                      </SelectContent>
                    </Select>
                    <FormMessage />
                  ...
                  <FormItem>
                    ...
                    <Select onValueChange={field.onChange} value={field.value}>
                      ...
                      <SelectContent>
                        ...
                        <SelectItem value="PENALTY_TICKET">
                          🎟️ Tiket Hapus Penalti
                        </SelectItem>
                      </SelectContent>
                    </Select>
                    <FormMessage />
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormDescription className="text-xs">
                      {rewardType === "NONE"
                        ? "Pilih tipe hadiah terlebih dahulu"
                        : rewardType === "NC"
                  ...
                  <FormItem className="w-full md:w-44 shrink-0">
                    <Select onValueChange={field.onChange} value={field.value}>
                      <FormControl>
                        <SelectTrigger className="bg-background border-border">
                          <SelectValue>
                            {TYPE_LABELS[field.value] || "Pilih Tipe"}
                          </SelectValue>
                        </SelectTrigger>
                      </FormControl>
                      <SelectContent>
  ...
  return (
    <div className="space-y-3 bg-muted/20 p-3 rounded-lg border border-dashed border-border mt-1">
                ...
                >
                  <FormControl>
                    <SelectTrigger className="h-7 bg-background border-border text-xs w-28">
                      <SelectValue>
                        {field.value === "OR" ? "Salah Satu (OR)" : "Semua (AND)"}
                      </SelectValue>
                    </SelectTrigger>
                  </FormControl>
                  <SelectContent>
      ...
      {fields.map((field, conditionIndex) => {
        ...
        const selectedQIndex = availableQuestions.findIndex(
          (q) => q.id === selectedQuestionId
        );
        ...
        const displayLabel = selectedQuestion
          ? `${selectedQIndex + 1}. ${selectedQuestion.questionText || "(belum diisi)"}`
          : "Pilih pertanyaan...";
        ...
        return (
          ...
          >
            ...
            {/* 3. Nilai — dropdown jika ada opsi, text input jika tidak */}
            <FormField
              control={control}
